/*
  ...
  1. Security Updates
    - Remove direct queries to auth.users table
    - Use JWT claims and auth.uid() for role checking
    - Implement proper RLS policies without recursion
    - Add helper function for role checking

  2. Changes
    - Create role checking function
    - Update all policies to use secure methods
    - Remove problematic auth.users queries
*/
...
CREATE POLICY "Teachers can manage attendance"
  ...
  USING (
    ...
    auth.user_role() IN ('teacher', 'admin') OR
    EXISTS (
      SELECT 1 FROM lectures l
      WHERE l.id = lecture_id AND l.teacher_id = auth.uid()
    )
